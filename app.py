...
<body>
  ...
  <script type="text/babel">
    ...
    // إصلاح استدعاء مكتبة ReactFlow
    const RF = window.ReactFlow;
    const ReactFlowComponent = RF.default || RF;
    const { Handle, Position, useNodesState, useEdgesState, Background, Controls } = RF;
      ...
      return (
        <div className={`custom-node ${selected ? 'selected' : ''}`}>
          ...
          <div className="node-title">
            <span>⚡ {data.label || 'عنصر كهربائي'}</span>
            <span className="node-badge">{r} Ω</span>
          </div>
    ...
    function App() {
      ...
      const addNode = () => {
        ...
        const newNode = {
          id: newId,
          type: 'component',
          position: { x: 150 + nodes.length * 30, y: 180 },
          data: { label: `مقاومة R${newId}`, voltage: 12, resistance: 20 }
        };
      ...
      return (
        <div className="app-layout">
          <div className="top-toolbar">
            <button className="btn-add" onClick={addNode}>➕ إضافة عنصر جديد</button>
          </div>
          ...
          <div className="properties-panel">
            ...
            {selectedNode ? (
              <>
                ...
                <div className="input-group">
                  <label>فولتية المصدر (Volt)</label>
                  <input type="number" value={v} onChange={(e) => handleChange('voltage', Number(e.target.value))} />
                </div>
                <div className="input-group">
                  <label>قيمة المقاومة (Ohm Ω)</label>
                  <input type="number" value={r} onChange={(e) => handleChange('resistance', Number(e.target.value))} />
                </div>
                <div className="metrics-card">
                  <div className="metrics-title">📊 التحليل الحسابي الفوري</div>
                  <div className="metric-row"><span>التيار المار (I):</span><strong>{i.toFixed(3)} A</strong></div>
                  <div className="metric-row"><span>القدرة المستهلكة (P):</span><strong>{p.toFixed(3)} W</strong></div>
                </div>
              </>
            ) : (
...
components.html(react_flow_html, height=780, scrolling=True)
